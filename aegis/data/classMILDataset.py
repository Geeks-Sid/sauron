    ...
    def __getitem__(
        ...
        if not self.use_hdf5:
            ...
            try:
                ...
                # 1. Get the dataset objects, but DO NOT read them into memory yet.
                features_dset = hdf5_file["features"]
                num_patches = features_dset.shape[0]

                # 2. Decide which indices to sample.
                if self.n_subsamples > 0 and num_patches > self.n_subsamples:
                    # Generate random indices WITHOUT loading the data
                    indices = torch.randperm(num_patches)[: self.n_subsamples].numpy()
                    indices.sort()  # Sorting indices can lead to faster reads from disk
                else:
                    # Use all patches
                    indices = np.arange(num_patches)
                # 3. Now, use these indices to read ONLY the required data from disk.
                # This is the efficient "fancy indexing".
                features = torch.from_numpy(features_dset[indices])

                # 4. Do the same for coordinates if they exist.

...
from sauron.feature_extraction.wsi.dataset import (
    WSIPatcherDataset,  # Use WSIPatcherDataset for dataloader
)
from sauron.feature_extraction.wsi.patching import (
...
class WSI:
    ...
    def create_patcher(
        ...
        if "openslide" in str(type(self)).lower():
            from sauron.feature_extraction.wsi.patching import OpenSlideWSIPatcher
            PatcherClass = OpenSlideWSIPatcher
        elif "cucim" in str(type(self)).lower():
            from sauron.feature_extraction.wsi.patching import CuImageWSIPatcher
            PatcherClass = CuImageWSIPatcher
        elif "image" in str(type(self)).lower(): # Assuming a PIL-based ImageWSI
            from sauron.feature_extraction.wsi.patching import NumpyWSIPatcher # Or a more specific one if available
    ...
    def visualize_coords(self, coords_path: str, save_patch_viz: str) -> str:
        ...
        try:
            ...
            warnings.warn(
                f"Cannot read using new Sauron coords format ({str(e)}). Trying with legacy CLAM/Fishing-Rod format."
            )
